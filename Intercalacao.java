...
public class Intercalacao extends Cliente {
    ...
    protected static ArrayList<Cliente> getAll(String filename){
        try{
    ...
    protected static void putRegister(Cliente c, String filename){
        try{
            RandomAccessFile raf = new RandomAccessFile(filename, "rw");
    ...
    protected static void putRegister(ArrayList<Cliente> c, String filename) { 
    ...
    protected static boolean isInOrder(ArrayList<Cliente> clientes){
        for(int i=0;i<clientes.size()-1;i++){
            int j=i+1;
            if(clientes.get(i).getID()>clientes.get(j).getID()){
                return false;
